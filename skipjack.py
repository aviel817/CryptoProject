# -- coding utf-8 --

# source: https://csrc.nist.gov/csrc/media/projects/cryptographic-algorithm-validation-program/documents/skipjack/skipjack.pdf

class SkipJack:
    def __init__(self):
        # F is an 8-bit S-box
        self.F = []
        self.defineF()
        # w1, w2, w3, w4 are 16-bit integers
        self.w1 = 0
        self.w2 = 0
        self.w3 = 0
        self.w4 = 0

    # plaintext is a 64-bit integer
    # key is a list of 10-bytes
    # 32 rounds
    # 2 rules rule A and rule B
    # first 8 rounds -rule A
    # second 8 rounds -rule B
    # third 8 rounds - rule A
    # forth 8 rounds - rule B
    def encrypt(self, plaintext, key):
        self.splitWord(plaintext)  # split into 4 words each word is 2 bytes(or 16 bits)

        # 32 rounds
        for round in range(1, 33):
            # round 1 and 3
            if (1 <= round <= 8) or (17 <= round <= 24):
                self.A(round, key)
                self.printStatus(round)
            # round 2 and 4
            if (9 <= round <= 16) or (25 <= round <= 32):
                self.B(round, key)
                self.printStatus(round)

        return self.appendWords()

    # ciphertext is a 64-bit integer
    # key is a list of 10-bytes
    def decrypt(self, ciphertext, key):
        self.splitWord(ciphertext)
        for round in reversed(range(1, 33)):
            if (25 <= round <= 32) or (9 <= round <= 16):
                self.Binv(round, key)
            if (17 <= round <= 24) or (1 <= round <= 8):
                self.Ainv(round, key)

        return self.appendWords()

    def A(self, round, key):
        c1 = self.w1
        c2 = self.w2
        c3 = self.w3

        # G permutes w1, the new w1 XOR counter XOR w4
        self.w1 = self.G(round, key, c1) ^ self.w4 ^ round
        self.w2 = self.G(round, key, c1)
        self.w3 = c2  # old w2
        self.w4 = c3  # old w3

    def Ainv(self, round, key):
        c1 = self.w1
        c2 = self.w2
        self.w1 = self.Ginv(round, key, c2)
        self.w2 = self.w3
        self.w3 = self.w4
        self.w4 = c1 ^ c2 ^ round

    def B(self, round, key):
        c1 = self.w1
        c2 = self.w2
        c3 = self.w3
        self.w1 = self.w4
        self.w2 = self.G(round, key, c1)
        self.w3 = c1 ^ c2 ^ round
        self.w4 = c3

    def Binv(self, round, key):
        c1 = self.w1
        self.w1 = self.Ginv(round, key, self.w2)
        self.w2 = self.Ginv(round, key, self.w2) ^ self.w3 ^ round
        self.w3 = self.w4
        self.w4 = c1

    # w is a 16-bit integer
    # The Gk box takes a 16-bit input and a 4-byte subkey k, and is itself a 4-round
    def G(self, round, key, w):
        g = [0] * 6
        g[0] = (w >> 8) & 0xff  # left half
        g[1] = w & 0xff  # right half
        j = (4 * (round - 1)) % 10

        for i in range(2, 6):  # gives i = 2,3,4,5
            g[i] = self.F[g[i - 1] ^ key[j]] ^ g[i - 2]
            j = (j + 1) % 10

        return (g[4] << 8) | g[5]

    # w is a 16-bit integer
    def Ginv(self, round, key, w):
        g = [0] * 6
        g[4] = (w >> 8) & 0xff
        g[5] = w & 0xff
        j = (4 * (round - 1) + 3) % 10

        for i in reversed(range(4)):  # gives i=3,2,1,0
            g[i] = self.F[g[i + 1] ^ key[j]] ^ g[i + 2]
            j = (j - 1) % 10

        return (g[0] << 8) | g[1]

    # append the four 16-bit words w1,w2,w3,w4 to return a 64-bit word
    def appendWords(self):
        x1 = self.w1 << 3 * 16
        x2 = self.w2 << 2 * 16
        x3 = self.w3 << 1 * 16
        x4 = self.w4
        return x1 | x2 | x3 | x4

    # w is a 64-bit word. This function splits w into
    # four 16-bit words which are stored in w1, w2, w3, w4
    def splitWord(self, w):
        self.w1 = (w >> (16 * 3)) & 0xffff
        self.w2 = (w >> (16 * 2)) & 0xffff
        self.w3 = (w >> (16 * 1)) & 0xffff
        self.w4 = w & 0xffff

    # print the round number and the current values of w1,w2,w3,w4
    def printStatus(self, round):
        w = self.appendWords()

    # print("round=" + str(round) + "  " + hex(w))

    # S(substitution)-Box of SkipJack
    def defineF(self):
        self.F = []


def encryptFile(imagePathToEncrypt, KEY):
    BLOCK_SIZE = 8  # 8 bytes = 64bit
    sj = SkipJack()
    with open(imagePathToEncrypt, 'rb') as f_in:
        with open('newFile_enc.jpg', 'wb') as f_out:
            block = f_in.read(BLOCK_SIZE)
            while block:
                block_encypt = sj.encrypt(int.from_bytes(block, 'big'), KEY)
                block_encypt_bytes = block_encypt.to_bytes(BLOCK_SIZE, 'big')
                f_out.write(block_encypt_bytes)
                block = f_in.read(BLOCK_SIZE)


def decryptFile(imagePathToDecrypt, KEY):
    BLOCK_SIZE = 8
    sj = SkipJack()
    with open(imagePathToDecrypt, 'rb') as f_in:
        with open('newFile_dec.jpg', 'wb') as f_out:
            block_encypt = f_in.read(BLOCK_SIZE)
            while block_encypt:
                block_encypt_int = int.from_bytes(block_encypt, 'big')
                block_int = sj.decrypt(block_encypt_int, KEY)
                block = block_int.to_bytes(BLOCK_SIZE, 'big')
                f_out.write(block)
                block_encypt = f_in.read(BLOCK_SIZE)
